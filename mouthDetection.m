function mouthMap = mouthDetection(YCrCb)
    ...
    % Make it binary
    mouthMapFormula = (mouthMapFormula - min(mouthMapFormula(:))) / ...
                      (max(mouthMapFormula(:)) - min(mouthMapFormula(:)));
    threshold = 0.4105; % Can be tweaked, but right now ok! 
    ...
    % Got the numbers by trail and error
    minArea = 40;   
    maxArea = 20000; 
    ...
    for i = 1:max(labeledMask(:))
        ...
        % Define ranges for the middle-bottom of the image
